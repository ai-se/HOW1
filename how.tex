...
none of those papers then go on to use this data to build {\em planners}.
...
HOW was inspired by some recent work by Krall and Menzies and Davis on 
the GALE optimizer~\cite{krall14}. A well-known effect in machine learning (and, in computer vision
...
towards the better end of each dimension $m_i$. 

A limitation with GALE is that whenever it adjusts an instance, some model has to be called
to re-evaluate the new objective scores for the new examples. This can be a limitation
since, in some domains, no such model exists or is readily accessible.
Building and certifying a model   takes a long time. Such slow-to-build models exist in software engineering. For  example, in previous work~\cite{me07f} we used models
...
might call HOW a spectral learner, 
a response surface method, or  a local search operator.
{\em Spectral learners}~\cite{kamvar03} reason across $m$ dimensions synthesized
...
variables~\cite{pearson1901}.  Usually,
...
Algorithms like PDDP~\cite{boley98} use PCA to
recursively divide data into smaller regions. Each
...
Platt has
  ...
  PCA~\cite{platt05}).
In step one, WHERE inputs
$N$
examples $N_1,N_2,..$. Secondly, WHERE
picks any
