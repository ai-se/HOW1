...
them. \fig{xxx} shows (1)~the training cases and (2)~the test cases and (3)~the
test cases displaced after applying HOW's plans for the {\em ant} data set.
In this figure, all the cases are projected onto two dimensions after (a)~creating one
data set with all of 1+2+3, finding their $x,y$ positions in the top-level {\tt cluster},
as generated by \fig{howcode}. Note that the both the test cases and the displaced test
cases all fall into nearly the same regions as the training data. This is to say that
predictors learned from the {\em ant} training data have some authority in the regions
used to generate our results\footnote{If the reader is concerned that this 2d projection
