...
then (3)~return the performance delta $\Delta$.
...
satisfactory} threshold used in this \fig{frame}. 
...
This study explores data sets from two types of domains:
\bi
\item
In one type, we 
...
by the {\em predictor}). Using engineering judgement, we define satisfactory recall as anything less than 70\%. 
\item 
...
Using engineering judgement, we define satisfactory $s$ as anything less than 70\%. 
\ei
...
Ant, Camel, Ivy, Jedit, JBeans, Log4, Lucene, Poi, Synapse, Velocity, Xalan, Xerces\footnote{Available from the object-oriented defects section of the PROMISE respository openscience.us/repo/defect/ck.} and five configuration data sets Berkeley DB, Apache, SQLite, LLVM,
 ...
 The median {\em before} recalls from  the static code data sets was:
%issue24
\[\mathrm{recall}=\{98,88,86,84,82,81,79,78,78,\overbrace{55,23,20}^{\mathrm{JBeans,Poi,Xalan}}\}\]
The median {\em before} $e$  scores from  the configuration data was
%issue 24
\[s=\{98,97,95,88,92\}\]
Note that we need to abort planning in only $3/17$ of our data sets.  

This paper also explores five configuration data sets Berkeley DB, Apache, SQLite, LLVM,
  x264. The median {\em before} scores from these data sets (generated in 
-- which means we cannot generate plans for $3/12$ of our data sets. However, as shown
below, we were able to generate useful plans for the other $9/12$ of the data.


In the other type, 


then studied to generate a prediction
on the test data. If the performance on that test data is not
satisfactory, thempredictions:
a {\em before} prediction (on the raw test data); and  an {\em after}
prediction (that is  applying the policies learned from the training data).
