...
\begin{document}
 ...
 For the test data used in this paper,
 we have observed that the improvements generated by HERE can reduce
 defect prediction and runtimes by up to
...
In the $21^{st}$ century, it is now impossible to manually browse all
the available software defect data.  For example, as of April 2015,
Mozilla Firefox has over a million bug reports on software projects.  While it is
...
\item Instead of just accepting  {\em predictions} on how many 
 software defects
to expect,  business users now demand {\em policies} that
...
{\em policies} that reduce that runtime.
\ei
In order to satisfy the demands of this new kind of business user, we  explore the generation of {\em policies} in the context
of software analytics. Such {\em policies} are recommendations of what
to change in order to better achieve some goal. 
Note that such {\em policy generation} is very different to the standard
data mining tasks of prediction some discrete or numeric class.
Predictors
and classifiers only tell us what {\em is} while policy 
generators tell us {\em what to change}. For the kind of business user
discussed above, policy generators are the prefeered tool since,
as one gruff user once shouted at us ``don't tell me what \underline{is};
...
\begin{small}
\[
project\;data \rightarrow \left\{\begin{array}{l}\mathit{train}
\rightarrow \left\{\begin{array}{l}\mathit{predictor\;(e.g.\;CART\;or\; RF)}\\
\mathit{policy\; generator\; (e.g.\; HOW)}\end{array}\right.
\\
~\\
\mathit{test}  \rightarrow  \left\{\begin{array}{r@{~}l}
\mathit{before}& =\mathit{predictor}(\mathit{test})\\
\mathit{after}& =\mathit{predictor}(\; \mathit{policy}(\mathit{test})\;)\\
