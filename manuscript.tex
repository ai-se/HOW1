...
\begin{document}
  \title{Temporal Transfer Learning with ``HERE'':\\ Local Policy Generation (in Near Linear Time)}
  ...
  \begin{abstract}
 Busy project managers grow impatient with analytcs
  \end{abstract}
  \begin{IEEEkeywords}
...
In order to satisfy these panelists, w  explore the generation of {\em policies} in the context
of software analytics. Such {\em policies} are recommendations of what
to change in order to better achieve some goal. In this paper the two
case studies are:
\bi
\item Learning code changes to static code features that reduce
the number of defects;
\item Learning reconfigurations of Makefiles such that, when the code
is compiled, that code runs faster.
\ei
Note that {\em policy generation} is different to the standard
data mining tasks of prediction or classification.  Predictors
and classifiers only tell us what {\em is} while policy 
generators tell us {\em what to do}.

The tool used to generate these policies is ``HERE'', an instance-based
local search algorithm that looks for improvement trends in the data,
then adjusting examples to nudge them along that improvement. While
